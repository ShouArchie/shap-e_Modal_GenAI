...
# The endpoint URL for your fresh deployment
API_ENDPOINT = "https://moulik-budhiraja--shape-text-to-3d-a100-40gb-generate.modal.run"
HEALTH_ENDPOINT = "https://moulik-budhiraja--shape-text-to-3d-a100-40gb-health.modal.run"

def check_cuda_health():
    """
    Check if CUDA is available on the server and report GPU information using nvidia-smi.
    
    Returns:
        bool: True if the health check was successful, False otherwise
    """
    print(f"Checking GPU availability at {HEALTH_ENDPOINT}")
        ...
        if response.status_code == 200:
            try:
                ...
                if 'cuda' in data:
                    ...
                    # Print detailed nvidia-smi information if available
                    if cuda.get('driver_version'):
                        print(f"  Driver Version: {cuda.get('driver_version', 'unknown')}")
                    if cuda.get('cuda_version'):
                        print(f"  CUDA Version: {cuda.get('cuda_version', 'unknown')}")
                    if cuda.get('device'):
                        print(f"  GPU: {cuda.get('device', 'unknown')}")
                    
                    # Print raw nvidia-smi output if available
                    if cuda.get('gpu_info'):
                        print("\nnvidia-smi output:")
                        print(cuda.get('gpu_info'))
                
                # Print warning if CUDA is not available
                if not cuda_available:
                    print("\nWARNING: GPU is not showing as available in the health check.")
                    print("This might be because:")
                    print("  1. The container is still initializing")
                    print("  2. The GPU wasn't properly allocated by Modal")
                    print("  3. The health endpoint doesn't have GPU access")
                    print("\nTry running the 'redeploy_a100_80gb.py' script to fix this issue.")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps", type=int, default=24, help="Number of diffusion steps (default: 24)")
    parser.add_argument("--seed", type=int, help="Random seed for reproducibility")
    parser.add_argument("--output", type=str, help="Output file path (default: based on prompt)")
    parser.add_argument("--health", action="store_true", help="Check CUDA availability and server health")
    
    args = parser.parse_args()
    
    # Check health if requested
    if args.health:
        health_ok = check_cuda_health()
        if not health_ok:
            sys.exit(1)
    ...
    # Ensure a prompt was provided if not just doing a health check
    if not args.prompt:
        parser.print_help()
